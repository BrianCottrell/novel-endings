class FifthStoryTextsController < ApplicationController
  $page = 1
  ...
  def index
    @fifth_story_texts = FifthStoryText.all
    @fifth_story_text = FifthStoryText.new
    ...
    if $update_post_time
      current_user.fifthpost_timer = Time.now.to_i
      current_user.update()
      $update_post_time = false
  ...
  def create
    @fifth_story_text = FifthStoryText.new(params.require(:fifth_story_text).permit(:passage))
    @fifth_story_text.author = current_user.email
...
#    @required_words = []
    ...
    @fifth_story_text.passage.bytes.each do |character|             #Check to makes sure each character is valid
    ...
    if ![33,34,46,63].include? @fifth_story_text.passage.bytes.last
    ...
    if @fifth_story_text.passage.bytes.first
      if @fifth_story_text.passage.bytes.first < 65 || @fifth_story_text.passage.bytes.first > 90
    ...
    @invalid_words.each do |word|                             #Check to see if any invalid words are included
      if @fifth_story_text.passage.bytes.each_cons(word.length).include? word
...
#        if @fifth_story_text.passage.bytes.each_cons(word.length).include? word.bytes
#          @valid = true
#        end
#      end
#      if !@valid
...
#    end
    ...
    if @valid                                                   #If the passage has not been rejected
      respond_to do |format|                                    #Add it to the story
        if(@fifth_story_text.save)
          format.html{redirect_to fifth_story_texts_path, notice: 'Story text was successfully created.'}
          format.json{render action: 'show', status: :created, location: @fifth_story_text}
          $update_post_time = true
        else
          format.html{render action: 'new'}
          format.json{render json: @fifth_story_text.errors, status: :unprocessable_entity}
        end
      end
    else
      redirect_to fifth_story_texts_path                              #Otherwise return
    end
  end
  
  #Repost an instance of a story text after it has been updated
  def update
    @fifth_story_text = FifthStoryText.find(params[:id])
    #
    respond_to do |format|
      if @fifth_story_text.update(params.require(:fifth_story_text, :passage))
        format.html{redirect_to @fifth_story_text, notice: 'Story text was successfully updated.'}
  ...
  #Delete a text entry
  def destroy
    @fifth_story_text = FifthStoryText.last
    @fifth_story_text.destroy
    $update_delete_time = true
    respond_to do |format|
      format.html { redirect_to fifth_story_texts_url }
      format.json { head :no_content }
    end
  end
end