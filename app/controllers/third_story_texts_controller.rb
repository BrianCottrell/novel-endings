class ThirdStoryTextsController < ApplicationController
  $page = 1
  ...
  def index
    @third_story_texts = ThirdStoryText.all
    @third_story_text = ThirdStoryText.new
    ...
    if $update_post_time
      current_user.third_post_timer = Time.now.to_i
      current_user.update()
      $update_post_time = false
  ...
  def create
    @third_story_text = ThirdStoryText.new(params.require(:third_story_text).permit(:passage))
    @third_story_text.author = current_user.email
    ...
    @required_words = ['Jamie','Carly','Oliver','Basil']
    $error_message = 'Enter your passage here:'                  
    #Filter process starts here
    @third_story_text.passage.bytes.each do |character|             #Check to makes sure each character is valid
    ...
    if ![33,34,46,63].include? @third_story_text.passage.bytes.last
    ...
    if @third_story_text.passage.bytes.first
      if @third_story_text.passage.bytes.first < 65 || @third_story_text.passage.bytes.first > 90
    ...
    @invalid_words.each do |word|                             #Check to see if any invalid words are included
      if @third_story_text.passage.bytes.each_cons(word.length).include? word
    ...
    if @valid
      ...
      @required_words.each do |word|
        if @third_story_text.passage.bytes.each_cons(word.length).include? word.bytes
          @valid = true
        end
      end
      if !@valid
    ...
    if @valid                                                   #If the passage has not been rejected
      respond_to do |format|                                    #Add it to the story
        if(@third_story_text.save)
          format.html{redirect_to third_story_texts_path, notice: 'Story text was successfully created.'}
          format.json{render action: 'show', status: :created, location: @third_story_text}
          $update_post_time = true
        else
          format.html{render action: 'new'}
          format.json{render json: @third_story_text.errors, status: :unprocessable_entity}
        end
      end
    else
      redirect_to third_story_texts_path                              #Otherwise return
    end
  end
  
  #Repost an instance of a story text after it has been updated
  def update
    @third_story_text = ThirdStoryText.find(params[:id])
    #
    respond_to do |format|
      if @third_story_text.update(params.require(:third_story_text, :passage))
        format.html{redirect_to @third_story_text, notice: 'Story text was successfully updated.'}
  ...
  #Delete a text entry
  def destroy
    @third_story_text = ThirdStoryText.last
    @third_story_text.destroy
    $update_delete_time = true
    respond_to do |format|
      format.html { redirect_to third_story_texts_url }
      format.json { head :no_content }
    end
  end
end