class SecondStoryTextsController < ApplicationController
  $page = 1
  ...
  def index
    @second_story_texts = SecondStoryText.all
    @second_story_text = SecondStoryText.new
    ...
    if $update_post_time
      current_user.second_post_timer = Time.now.to_i
      current_user.update()
      $update_post_time = false
  ...
  def create
    @second_story_text = SecondStoryText.new(params.require(:second_story_text).permit(:passage))
    @second_story_text.author = current_user.email
    ...
    @required_words = ['Alex','Alfonzo','Grant','Lorin','Meredith','Richard','Zach']
    
    $error_message = 'Enter your passage here:'                  
    #Filter process starts here
    @second_story_text.passage.bytes.each do |character|             #Check to makes sure each character is valid
    ...
    if ![33,34,46,63].include? @second_story_text.passage.bytes.last
    ...
    if @second_story_text.passage.bytes.first
      if @second_story_text.passage.bytes.first < 65 || @second_story_text.passage.bytes.first > 90
    ...
    @invalid_words.each do |word|                             #Check to see if any invalid words are included
      if @second_story_text.passage.bytes.each_cons(word.length).include? word
    ...
    if @valid
      ...
      @required_words.each do |word|
        if @second_story_text.passage.bytes.each_cons(word.length).include? word.bytes
          @valid = true
        end
      end
      if !@valid
    ...
    if @valid                                                   #If the passage has not been rejected
      respond_to do |format|                                    #Add it to the story
        if(@second_story_text.save)
          format.html{redirect_to second_story_texts_path, notice: 'Story text was successfully created.'}
          format.json{render action: 'show', status: :created, location: @second_story_text}
          $update_post_time = true
        else
          format.html{render action: 'new'}
          format.json{render json: @second_story_text.errors, status: :unprocessable_entity}
        end
      end
    else
      redirect_to second_story_texts_path                              #Otherwise return
    end
  end
  
  #Repost an instance of a story text after it has been updated
  def update
    @second_story_text = SecondStoryText.find(params[:id])
    #
    respond_to do |format|
      if @second_story_text.update(params.require(:second_story_text, :passage))
        format.html{redirect_to @second_story_text, notice: 'Story text was successfully updated.'}
  ...
  #Delete a text entry
  def destroy
    @second_story_text = SecondStoryText.last
    @second_story_text.destroy
    $update_delete_time = true
    respond_to do |format|
      format.html { redirect_to second_story_texts_url }
      format.json { head :no_content }
    end
  end
end